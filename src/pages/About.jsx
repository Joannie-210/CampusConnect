...
const About = () => {
  ...
  <div className="about-card-overview">
    {[
      {
        img: Aptech_Limited_Logo,
        title: "Aptech Institute",
        desc: "Excellence in global tech education",
      },
      {
        img: aptechlocation,
        title: "Location",
        desc: "MBM Plaza, Port Harcourt, Rivers State",
      },
      {
        img: middleSex,
        title: "Affiliation",
        desc: "Middlesex University, London",
      },
    ].map((card, i) => (
      <article
        key={i}
      ...
      <section className="about-highlights">
        <h2>Campus <i style={{fontStyle: "normal", color: "#ffaa00"}}>Highlights</i> & Recognitions</h2>
        <p>
          Proudly recognized for academic excellence, innovation, and student life.
        </p>
        <ul>
      ...
      <section className="about-event">
        ...
        <div className="about-event-container">
          {aboutevents.map((ev) => (
            <div key={ev.id} className="AevenetCard">
              <img src={ev.image} alt={ev.title} />
              <div className="Aevent-lay">
                <h3>{ev.title}</h3>
                <p style={{color: "#cdcdcd", textAlign: "left", fontSize: "14px", fontFamily: "sans-serif"}}>{ev.description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="about-timelines">
        ...
        <div className="acounttimelines">
          {timelines.map((t, i) => (
            <div key={i} className="about-timelines-item">
